...
export const actions = {
    async fetchAllItems({ commit }) {
        ...
        try {
            const items = await this.$axios.$get('depoDetay/List')
    ...
    async fetchItems({commit},item)
    {
        commit('setLoading',true)
        try{
            const response = await this.$axios.$get('depoDetay/PagedList',{ params: item, paramsSerializer: (params) => { return qs.stringify(Object.fromEntries(Object.entries(params).filter(([k, v]) => v), { arrayFormat: 'repeat', skipNulls: true }), { indices: false }) } })
    ...
    async getItems(context){
        ...
        const res = await this.$axios.$get(`depoDetay/Get/${item.id}`)
    ...
    async insertOrUpdateItem({ commit }, item) {
        ...
        try {
            if (item.id === 0) {
                const response = await this.$axios.$post('depoDetay/Add', item)
                commit('addItem', response.value)
                commit('setSuccedItem',response.value)
            } else {
                const response = await this.$axios.$put('depoDetay/Update', item)
                commit('updateItem', response.value)
                commit('setSuccedItem',response.value)
            }
            commit('setLoading', false)
        } catch (error) {
            commit('setSuccedItem',{depo:{id:0}})
        }
    },
    async deleteItem({ commit }, item) {
        commit('setLoading', true)
        try {
            const res = await this.$axios.$delete(`depoDetay/Delete/${item.id}`)
